...
import { getPageUrl } from "@/app/config";
...
export async function createRoomWithLexicalDocument(markdown: string, title?: string) {
  ...
  await withLexicalDocument(
    {
      roomId: room.id,
      client: liveblocks,
      nodes: [CodeNode, LinkNode, ListNode, ListItemNode, HeadingNode, QuoteNode],
    },
    async (doc) => {
      await doc.update(() => {
